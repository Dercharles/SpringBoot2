...
public class ImageUtil {
	...
	public static String saveImgByPoint(int x, int y, int w, int h, String filePath, String fileType,String subdomain) throws Exception {
        try {
            ...
            String[] destPathArr = UploadUtil.GetCreatePathWithSuffix(filePath, null,subdomain);
            ImageIO.write(out, fileType, new File(destPathArr[0]));

            return destPathArr[1];
        }catch (Exception e){
            log.error("saveImgByPoint error,filePath="+filePath);
            throw e;
        }
	}
