package com.example.springboot2.yang.common.utils;
	...
	public static Map<String, SearchFilter> parse(Map<String, Object> searchParams) throws Exception{
		...
		if(searchParams!=null){
			for (Entry<String, Object> entry : searchParams.entrySet()) {
				...
				if (names.length == 1) {
					filedName = names[0];
					operator = SearchFilter.Operator.valueOf("EQ");
				}else if (names.length == 2) {
					filedName = names[1];
					operator = SearchFilter.Operator.valueOf(names[0]);
				}else{
					throw new IllegalArgumentException(key + " is not a valid search filter name");
				}
...
//						}
//					}else{
//                        value= DateUtil.convertDateMutip(value.toString());
//					}
//
//				}
