package com.example.springboot2.yang.common.modules.email;


import com.example.springboot2.yang.common.mail.MailBean;
...
//			System.out.println(content);
			...
			List<AttachmentVo> attachments = mailBean.getAttachments();
            for (AttachmentVo vo: attachments) {
				String path = vo.getFullPath();
				if(Validator.isNull(path)){
					if(Validator.isNotNull(vo.getRelativePath())){
						path = "e://" + vo.getRelativePath();
					}else {
						continue;
					}
				}
                helper.addAttachment(vo.getRealName(), new File(path));
            }
			mailSender.send(msg);
