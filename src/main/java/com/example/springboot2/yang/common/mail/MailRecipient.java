...
import com.example.springboot2.yang.common.modules.utils.Encodes;
		...
		if(folder != null){
			...
			for (int i = 0; i < msgArr.length; i++) {
				...
				if (flags.contains(Flags.Flag.SEEN)){
					 //logger.info("mail job--8");
					continue;
                }
                try {
					MailBean mailBean = _convert(mimeMessage);
					if (mailBean == null) {
						logger.info("mail job--13--mailBean is null");
						break;
					}
	...
	private MailBean _convert(MimeMessage msg) throws MessagingException {
		...
		try{
            ...
            try {
				...
				mailBean.getToAddresses().add("channel name");
			}
...
//            InternetAddress[] bccs = (InternetAddress[])msg.getRecipients(RecipientType.BCC);
...
//            }
            ...
            try {
                _convertMessageBody((Part)msg,mailBean);
				 logger.info("mail job--11");
            } catch (Exception e) {
                logger.error("get email content error!", e);
            }
			 logger.info("mail job--12");
            return mailBean;
        }catch (MessagingException me){
			logger.error("MailRecipient _convert error!",me);
			return null;
		}
	}
	...
	private void _convertMessageBody(Part part,MailBean mailBean) throws MessagingException, IOException,Exception {
		 logger.info("mail job--10");
		if(part.isMimeType("text/plain")){
			if(part.getDisposition() != null && part.getFileName()!= null){
				String s2 = part.getFileName().replace("?gb2312?", "?gbk?");
				String strFileName = MimeUtility.decodeText(s2); //MimeUtility.decodeText解决附件名乱码问题
//				System.out.println("发现附件: " +  strFileName);
//				System.out.println("内容类型: " + MimeUtility.decodeText(part.getContentType()));
//				System.out.println("附件内容:" + MimeUtility.decodeText((String)part.getContent()));
				//ThreadManage.threadAssign(TheadConstants.THREAD_POOL_SMALL, this, "processAttachment", strFileName, part.getInputStream(),mailBean);
				this.processAttachment(strFileName, part.getInputStream(),mailBean);
			}else{
				mailBean.setContentText(new MailBodyPart((Part) part).getContent());
			}
		}else if(part.isMimeType("text/html")){
			mailBean.setContentHtml(new MailBodyPart((Part) part).getContent());
			...
			//ThreadManage.threadAssign(TheadConstants.THREAD_POOL_SMALL, this, "processAttachment", part,mailBean);
	...
	private void processAttachment(Part part,MailBean mailBean) throws MessagingException, IOException{
		...
		if((disposition != null && (Part.INLINE.equalsIgnoreCase(disposition)
			...
			if (fileName == null){
				fileName = "untitled["+(int)(Math.random()*(9999-1000+1))+1000+"].bak";
			}
			if(null != fileName){
				String s2 = fileName.replace("?gb2312?", "?gbk?");
				fileName = MimeUtility.decodeText(s2);
			}
...
//			System.out.println("--folder="+folder);
			if(downloadFile&&path!=null){
				...
				try {
					...
					state=false;
					logger.info("downloadFile out:" + mailBean.getSender()+new Date());
				}
			}
		}
	}
...
//		System.out.println("---path:"+folder+File.separatorChar+fileName);
	...
	public static void SetLogMap(Long orgId,Boolean onOff){
		LogMap.put(orgId,onOff);
	}
}
