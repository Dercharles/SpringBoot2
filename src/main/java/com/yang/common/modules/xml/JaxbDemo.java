...
import com.visionet.letsdesk.app.channel.vo.weixin.WeixinResponseVo;
...
public class JaxbDemo {
    ...
    public static void main(String[] args) {
        try{
            WeixinResponseVo vo = new WeixinResponseVo();
            vo.setContent("测试1");
            vo.setCreateTime(new Date().getTime());
            vo.setToUserName("xxxxx1");
            vo.setFromUserName("xxxxx2");
            vo.setMsgType("type1");

            System.out.println(JaxbMapper.toXml(vo));
        }catch (Exception e){
            e.printStackTrace();
        }
    }
