package com.yang.common.modules.string;


import com.yang.common.base.rest.RestException;
import com.yang.common.file.FileUtil;
import com.yang.common.modules.MessageSourceHelper;
...
//	}
	...
	public static String[] toStringArray(String add[],Object[] objects){
		String[] str = new String[objects.length+add.length];
		for(int i=0;i<objects.length + add.length;i++){
			if(i<add.length) {
				str[i] = add[i] == null ? "" : add[i].toString();
			}else {
				str[i] = objects[i-add.length] == null ? "" : objects[i-add.length].toString();
			}
		}
		return str;
	}
...
//            processAutoIncrement("/Users/xuetao/Downloads/dmp/wsk_ddl.sql","/Users/xuetao/Downloads/dmp/wsk_ddl3.sql");
			...
			System.out.println(getRandomNumber(899)+100);
		}catch (Exception e){
            e.printStackTrace();
        }
    }
